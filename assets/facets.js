...
class FacetsFormComponent extends Component {
  ...
  createURLParameters(formData = new FormData(this.refs.facetsForm)) {
    ...
    for (const [key, value] of formData.entries()) {
      newParameters.append(key, String(value));
    }
    ...
    // Mevcut URL'deki hierarchy_paths parametresini koru
    const currentUrl = new URL(window.location.href);
    const hierarchyParam = currentUrl.searchParams.get('filter.p.m.custom.hierarchy_paths');
    if (hierarchyParam && !newParameters.has('filter.p.m.custom.hierarchy_paths')) {
      newParameters.set('filter.p.m.custom.hierarchy_paths', hierarchyParam);
    }
...
class FacetRemoveComponent extends Component {
  ...
  removeFilter({ form }, event) {
    ...
    // Clear all butonuysa ve category parametresi varsa, onu koru
    const isClearAll = this.querySelector('.facets__clear-all-link') !== null;
    if (isClearAll) {
      const currentUrl = new URL(window.location.href);
      const hierarchyParam = currentUrl.searchParams.get('filter.p.m.custom.hierarchy_paths');
      ...
      if (hierarchyParam) {
        ...
        targetUrl.searchParams.set('filter.p.m.custom.hierarchy_paths', hierarchyParam);
...
/**
 * Handles horizontal filter click events to prevent default and open vertical drawer
 * Only applies to actual filter panels, not sorting or other controls
 */
window.addEventListener('load', () => {
  // Prevent horizontal filters from opening their dropdown panels
  document.addEventListener('click', (event) => {
    if (!(event.target instanceof HTMLElement)) return;
    
    const summary = event.target.closest('.facets__summary');
    if (!summary) return;
    
    const details = summary.closest('details');
    if (!details) return;
    
    // Check if this is a horizontal filter but NOT a sorting filter
    if (details.closest('.facets--horizontal') && !details.closest('.sorting-filter')) {
      // Prevent the details from opening
      event.preventDefault();
      
      // Close the details if it's open
      details.open = false;
      
      // Open the vertical filters drawer instead
      const verticalDrawer = document.getElementById('vertical-filters-drawer');
      if (verticalDrawer && 'showDialog' in verticalDrawer) {
        // @ts-ignore
        verticalDrawer.showDialog();
      }
    }
  });
});
