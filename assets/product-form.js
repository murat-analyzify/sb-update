...
export class AddToCartComponent extends Component {
   ...
   */
  handleClick(event) {
    // Check form validity before animating (for gift cards with required email)
...
class ProductFormComponent extends Component {
  ...
  handleSubmit(event) {
    ...
    // Check form validity before proceeding (for gift cards with required email)
    ...
    fetch(Theme.routes.root + 'cart/add.js', {
      ...
      .then((response) => {
        if (response.status) {
          ...
          // except stanley and jellcat vendors
          const productVendorElement = this.dataset.productVendor;
          const vendorName = productVendorElement?.toUpperCase() || '';
          const isStanleyOrJellycat = vendorName === 'STANLEY' || vendorName === 'JELLYCAT';

          if (!isStanleyOrJellycat) {
            this.dispatchEvent(
 ...
 */
  #resetQuantityToOne() {
    ...
    const quantityInput = form.querySelector('input[ref="quantityInput"]');
